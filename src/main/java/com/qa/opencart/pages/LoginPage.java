...
public class LoginPage {
	...
	public boolean doInvalidLogin(String username, String password2) {
		eleUtil.waitForElementVisible(email, MEDIUM_DEFAULT_TIMEOUT).sendKeys(username);
		eleUtil.doSendKeys(password, password2);
		eleUtil.doClick(loginBtn);
		String errorMesg = eleUtil.waitForElementVisible(By.cssSelector("div.alert.alert"), DEFAULT_TIMEOUT).getText();
		System.out.println("login error mesg: " + errorMesg);
		return false;
	}
